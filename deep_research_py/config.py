...
class ProviderConfig:
    ...
    def api_key(self) -> Optional[str]:
        ...
        if self.service_provider_name == ServiceProvider.OLLAMA.value:
            return "no_api_key" # Have to send some string to the client to not get error
...
class EnvironmentConfig:
    ...
    @classmethod
    def get_provider_config(cls, service_provider_name: Optional[str] = None) -> ProviderConfig:
        ...
        if not provider:
            raise ValueError("No provider specified and DEFAULT_SERVICE not set in environment")
    ...
    @classmethod
    def validate_provider_config(cls, service_provider_name: Optional[str] = None, console: Optional[Console] = None) -> ProviderConfig:
        ...
        try:
            ...
            if not config.model:
                console.print(f"[yellow]Warning: No model specified for {config.service_provider_name}[/yellow]")
